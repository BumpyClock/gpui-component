    ...
    D: TableDelegate,
{
    ...
    /// Set to enable/disable cell selectable, default false
        ...
        if row_ix < rows_count {
            ...
            tr.h_flex()
                    ...
                    h_flex()
                        ...
                        .child({
                            ...
                            for col_ix in visible_col_range.clone() {
                                ...
                                let is_cell_selected = self.selected_cell
                                    == Some((row_ix, actual_col_ix))
                                    && self.selection_mode.is_cell();
                                let is_cell_right_clicked =
                                    self.right_clicked_cell == Some((row_ix, actual_col_ix));

                                row_cols = row_cols.child(
                                    self.render_col_wrap(Some(row_ix), actual_col_ix, window, cx)
                                        .child(
                                            self.render_cell(
                                                Some(row_ix),
                                                actual_col_ix,
                                            ...
                                            .id(format!("table-cell:{}:{}", row_ix, actual_col_ix))
                                            .relative()
                                            .child(self.measure_render_td(
                                                row_ix,
                                                actual_col_ix,
                ...
                .when_some(self.selected_row, |this, _| {
                    this.when(
                        is_selected && self.selection_mode == SelectionMode::Row,
                        |this| {
                            this.map(|this| {
