...
///
/// Registration is *dynamic once the plugin is live*: because [`AppPlugin`] is
...
mod tests {
    ...
    fn register_dedups_last_wins_and_keeps_slot() {
        ...
        reg.register(cmd("a", Alpha).with_binding("cmd-a"));
        reg.register(cmd("b", Beta));
